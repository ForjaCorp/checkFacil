...
function CompleteEventDetailsPage() {
    ...
    const { data: eventData, isLoading: isFetching, isError: pageError } = useQuery<ApiEventData>({
    ...
    useEffect(() => {
        if (eventData) {
            const formValuesToSet = {
                ...
                partyDate: eventData.data_festa ? new Date(eventData.data_festa.replace(/-/g, '/')) : new Date(),
    ...
    if (pageError) {
        return <div className="text-center p-8"><p className="text-destructive">Não foi possível carregar os detalhes do evento.</p><Button variant="outline" onClick={() => navigate(-1)} className="mt-4">Voltar</Button></div>;
    }
            ...
            <Card>
                <CardHeader>
                    <div className='flex justify-between items-start'>
                        <div>
                            <CardTitle className="hidden text-2xl font-bold lg:block"><h1>Complete os Detalhes da Sua Festa</h1></CardTitle>
                            <CardDescription>Revise e preencha as informações abaixo para finalizar o agendamento.</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                <CardContent>
                            ...
                            <div className="space-y-4 rounded-md border p-4">
                                ...
                                {eventData?.link_convite ? (
                                    <div className="my-4">
                                        ...
                                        <img src={eventData.link_convite} alt="Convite da Festa" className="rounded-md max-h-60 w-auto" />
                                    </div>
                                ) : (
                                    <p className="text-sm text-muted-foreground py-2">Nenhum convite enviado ainda.</p>
                                )}
                                <p className="text-sm text-muted-foreground">{eventData?.link_convite ? 'Para alterar, envie uma nova imagem:' : 'Faça o upload da imagem que será compartilhada:'}</p>
                                <Input type="file" accept="image/png, image/jpeg" onChange={handleInviteImageUpload} disabled={isUploading || isSaving} />
                                {isUploading && <p className="text-sm text-primary flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Enviando imagem...</p>}
                            </div>
                            <Button type="submit" className="w-full" disabled={isSaving || isUploading}>
                                {(isSaving || isUploading) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                                {isSaving ? 'Salvando...' : isUploading ? 'Aguarde o upload...' : eventData?.status === 'RASCUNHO' ? 'Finalizar Agendamento e Salvar' : 'Salvar Alterações'}
                            </Button>
                        </form>
                    </Form>
                </CardContent>
